...
PageManager.prototype.start = function () {
    this.enableKeyboardNavigation(); // Enable navigation with the right arrow key
    for (var i = 0; i < this.pages.length; ++i) {
...
PageManager.prototype.enableKeyboardNavigation = function () {
  document.addEventListener("keydown", (event) => {
    const currentPage = this.getCurrentPage();
    if (event.key === "ArrowLeft" && this.pagesIndex === 0) {
      return; // Prevent left navigation if already on the first page
    }

    if (currentPage instanceof ConsentPage || currentPage instanceof FinishPage) {
      return; // Prevent navigation if on ConsentPage or FinishPage
    }
