...
FinishPage.prototype.render = function (_parent) {
    ...
    for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
      ...
      } else if (element.type === "classcode") {
        table.append($("<tr><td><strong>"+ element.label +"</strong></td><td><input id='"+element.name+"' /></td></tr>"));
...
function calculateChecksum(code) {
  ...
  const checksumIndex = sum % validChars.length;
  return validChars[checksumIndex];
}

function validateCode(codeWithChecksum) {
  const code = codeWithChecksum.slice(0, 7); // Extract the code (without checksum)
  const checksum = codeWithChecksum.slice(7); // Extract the checksum
  const calculatedChecksum = calculateChecksum(code);
  // Check if first four characters are digits and the next three are lowercase letters
  if (/^\d{4}[a-z]{3}$/.test(code)) {
    return checksum === calculatedChecksum; // Check if checksum is valid
  }
    ...
    this.interval = setInterval((function(){ 
      ...
      for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
        ...
        if (element.type === "text") {
          ...
          var value = $("#" + element.name).val();
          // var regex = /^\d{4}[a-zA-Z]{4}$/;  // 4 digits followed by 4 letters
          if ((value && validateCode(value)) || element.optional == true) {
            ++counter;
          }
        } else if (element.type === "number") {
          if ($("#" + element.name).val() || element.optional == true) {
