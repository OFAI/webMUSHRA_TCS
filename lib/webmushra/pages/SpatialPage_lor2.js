			...
			if ((newtime - this.page.time) >= this.page.framerate) {
				...
				if (this.page.mouseDown == true) {
					if (this.page.activeResponse !== null) {
						...
						// Logging the calculated positions for debugging
						console.log(`Calculated mouse position: ${positions[0]}, ${positions[1]}, ${positions[2]}`);
		
						for (var j = 0; j < objects.length; ++j) {
							if (objects[j].eventOnMouseMove && this.scene.camera.type !== "PerspectiveCamera") {
								console.log(`Moving object: ${objects[j]} to position: ${positions[0]}, ${positions[1]}, ${positions[2]}`);
