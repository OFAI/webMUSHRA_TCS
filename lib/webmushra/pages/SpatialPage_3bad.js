    ...
    for (var i = 0; i < this.pageConfig.views.length; ++i) {
		...
		img.onload = (function(ctx, canvas, page) {
			return function() {
				...
				// Ensure ctx is defined before trying to draw
				if (ctx) {
		...
		canvas.onmousedown = (function(event) {
			...
			var relX_canvas = event.clientX - rect.left;
			var relY_canvas = event.clientY - rect.top;

			this.page.circleX = relX_canvas;
			this.page.circleY = relY_canvas;
        ...
        if (this.pageConfig.views[i].view === 'front' || this.pageConfig.views[i].view === 'back' || this.pageConfig.views[i].view === 'top' || this.pageConfig.views[i].view === 'right' || this.pageConfig.views[i].view === 'left') {
            ...
            // img.src = '/res/spatial/Austria_map_with_states.png';  // Adjust the path as needed
