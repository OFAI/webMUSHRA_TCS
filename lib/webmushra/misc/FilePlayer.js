...
FilePlayer.prototype.init = function() {
  ...
  this.enableKeyboardPlay(); // Enable play with the space key
...
FilePlayer.prototype.enableKeyboardPlay = function () {
  document.addEventListener("keydown", (event) => {
    if (event.key === " ") { // Space key
      event.preventDefault(); // Prevent default scrolling behavior
      this.genericAudioControl.play(0); // Play the first stimulus by default
    }
  });
};
